...
exports.updateFeverWithRash = async (casebase_id, disease_id, data) => {
  const result = await pool.query(
    `UPDATE tbl_feverwithrash SET
      mr_mmr_vaccinated=$1, fever=$2, rash=$3, cough=$4,
      runny_nose=$5, red_eyes=$6, joint_pain=$7,
      lymph_nodes=$8, other_symptoms=$9, patient_status=$10,
      updated_at=NOW()
...
exports.deleteFeverWithRash = async (casebase_id, disease_id) => {
  await pool.query(
    `DELETE FROM tbl_feverwithrash WHERE casebase_id=$1 AND disease_id=$2`,
    [casebase_id, disease_id]
  );
};